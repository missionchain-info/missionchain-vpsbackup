...
/// @title OperationalSalaryPoolV3 — Policy contract for SEED Operational slot (20%)
/// @notice
...
///   `SeedBudgetV5c.release(SLOT_OPERATIONAL, member, amount)` which transfers
...
/// @notice V3 redeploy points to SeedBudgetV5c. Same logic as V2.
contract OperationalSalaryPoolV3 is ReentrancyGuard {
    ...
    modifier onlyOwner() { require(msg.sender == owner, "OSPv3: not owner"); _; }

    constructor(address _council, address _seedBudget, address _owner) {
        require(_council    != address(0), "OSPv3: zero council");
        require(_seedBudget != address(0), "OSPv3: zero seedBudget");
        require(_owner      != address(0), "OSPv3: zero owner");
    ...
    function enrollMember(
        ...
        require(council.isActiveMember(wallet), "OSPv3: not council member");
        require(!members[wallet].enrolled, "OSPv3: already enrolled");
        require(sharePctBps > 0, "OSPv3: zero share");
        require(uint256(totalShareBps) + sharePctBps <= 10_000, "OSPv3: total >100%");
    ...
    function updateMember(
        ...
        require(m.enrolled, "OSPv3: not enrolled");
        uint16 newTotal = totalShareBps - m.sharePctBps + newSharePctBps;
        require(newTotal <= 10_000, "OSPv3: total >100%");
    ...
    function removeMember(address wallet) external onlyOwner {
        ...
        require(idx1 != 0, "OSPv3: not enrolled");
        PoolMember storage m = members[wallet];
    ...
    function claim() external nonReentrant {
        ...
        require(amount > 0, "OSPv3: nothing to claim");
