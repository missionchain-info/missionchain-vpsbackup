...
/// @title ManagementBonusPoolV2 — Policy contract for SEED Mgmt Bonus slot (10%)
/// @notice
...
///   SeedBudgetV5b.release(SLOT_MGMT_BONUS, recipient, amount) to disburse.
contract ManagementBonusPoolV2 is ReentrancyGuard {
    ...
    modifier onlyOwner()    { require(msg.sender == owner, "MBPv2: not owner"); _; }
    modifier onlyCouncil()  { require(council.isActiveMember(msg.sender), "MBPv2: not active council"); _; }

    constructor(address _council, address _seedBudget, address _owner) {
        require(_council    != address(0), "MBPv2: zero council");
        require(_seedBudget != address(0), "MBPv2: zero seedBudget");
        require(_owner      != address(0), "MBPv2: zero owner");
    ...
    function setThreshold(uint16 newBps) external onlyOwner {
        require(newBps > 0 && newBps <= 10_000, "MBPv2: invalid bps");
    ...
    function createOrder(address recipient, uint256 amount, string calldata content)
        ...
        returns (uint256 id)
    {
        require(recipient != address(0), "MBPv2: zero recipient");
        require(amount > 0, "MBPv2: zero amount");
    ...
    function approveOrder(uint256 id) external onlyCouncil {
        ...
        require(o.id == id && id != 0, "MBPv2: invalid order");
        require(o.status == OrderStatus.PENDING, "MBPv2: not pending");
        require(!hasVoted[id][msg.sender], "MBPv2: already voted");
    ...
    function executeOrder(uint256 id) external nonReentrant {
        ...
        require(active > 0, "MBPv2: no active council");
        require(approvalsCount[id] * 10_000 >= active * thresholdBps, "MBPv2: threshold not met");
        require(seedBudget.slotBalance(SLOT_MGMT_BONUS) >= o.amount, "MBPv2: insufficient slot balance");
