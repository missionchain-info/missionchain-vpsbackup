...
/// @title LiquidityPoolV7 — protocol MIC/USDT pool, linear buy price
...
contract LiquidityPoolV7 is AccessControl, ReentrancyGuard {
    ...
    constructor(
        ...
        require(_usdt != address(0) && _mic != address(0), "LP7: zero token");
        require(admin != address(0), "LP7: zero admin");
        require(_virtualReserve0 > 0, "LP7: zero virtual reserve");
        require(_sellGateUsdt > 0, "LP7: zero sell gate");
        // A gate above the virtual reserve could never be reached by substitution alone.
        require(_sellGateUsdt <= _virtualReserve0, "LP7: gate above virtual reserve");
        ...
        require(IERC20Metadata(_usdt).decimals() == 18, "LP7: usdt must be 18 decimals");
    ...
    function seedMic(uint256 amount) external onlyRole(DEFAULT_ADMIN_ROLE) nonReentrant {
        require(amount > 0, "LP7: zero amount");
    ...
    {
        require(startTime != 0, "LP7: pool not seeded");
        require(usdtIn > 0, "LP7: zero amount");
        require(_lastTradeBlock[msg.sender] != block.number, "LP7: same block");
        _accrue();
        ...
        require(micOut >= minMicOut, "LP7: slippage");
        require(micOut > 0, "LP7: zero out");
        require(micOut <= (reserveMic * MAX_TRADE_BPS) / BPS, "LP7: over per-trade limit");
    ...
    {
        require(startTime != 0, "LP7: pool not seeded");
        require(phase != Phase.Bootstrap, "LP7: sells not open");
        require(micIn > 0, "LP7: zero amount");
        require(_lastTradeBlock[msg.sender] != block.number, "LP7: same block");
        require(micIn <= (reserveMic * MAX_TRADE_BPS) / BPS, "LP7: over per-trade limit");
        _accrue();
        ...
        require(usdtOut > 0, "LP7: zero out");
        require(usdtOut >= minUsdtOut, "LP7: slippage");
        ...
        require(usdtOut <= usdt.balanceOf(address(this)), "LP7: insufficient balance");
        require(usdtOut <= reserveUsdt, "LP7: insufficient reserve");
    ...
    {
        require(recipient != address(0), "LP7: zero recipient");
        require(amount > 0 && amount <= reserveMic, "LP7: bad amount");
    ...
    function executeMicWithdraw() external onlyRole(DEFAULT_ADMIN_ROLE) nonReentrant {
        ...
        require(w.amount > 0, "LP7: no request");
        require(block.timestamp >= w.executableAt, "LP7: cooldown active");
        // The float can have shrunk through sales since the announcement.
        require(w.amount <= reserveMic, "LP7: insufficient reserve");
    ...
    function _checkAndBookOutflow(uint256 amount) internal {
        ...
        require(usdtOutInWindow + amount <= cap, "LP7: over daily limit");
