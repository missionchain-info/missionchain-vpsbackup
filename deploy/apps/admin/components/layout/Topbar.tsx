...
function getStoredTheme(): AdminTheme {
  ...
  const t = localStorage.getItem('mc-admin-theme-v2') as AdminTheme | null;
  return t && THEME_ORDER.includes(t) ? t : 'light';
}
...
export default function Topbar({ onToggleSidebar }: TopbarProps) {
  ...
  const handleDisconnect = useCallback(() => {
    ...
    localStorage.removeItem('mc-admin-theme-v2');
    router.push('/login');
  }, [router]);
  ...
  const toggleTheme = useCallback(() => {
    ...
    localStorage.setItem('mc-admin-theme-v2', next);
  }, [theme]);
  ...
  return (
    <div className="topbar">
      ...
      <button
        ...
        title="Disconnect"
        style={{
          background: 'rgba(217,79,96,0.12)',
          border: '1px solid rgba(217,79,96,0.3)',
          borderRadius: 6,
          padding: '4px 10px',
          cursor: 'pointer',
          color: '#D94F60',
