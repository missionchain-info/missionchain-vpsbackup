...
const TONE: Record<string, { bg: string; fg: string; label: string }> = {
  ...
  warning:  { bg: 'rgba(240,181,74,.14)',  fg: 'var(--warning)', label: 'DUE SOON' },
  critical: { bg: 'rgba(242,109,139,.14)', fg: 'var(--error)',   label: 'URGENT' },
  expired:  { bg: 'rgba(242,109,139,.22)', fg: 'var(--error)',   label: 'EXPIRED' },
  unset:    { bg: 'rgba(240,181,74,.10)',  fg: 'var(--warning)', label: 'NO DATE SET' },
  unknown:  { bg: 'rgba(150,137,194,.14)', fg: 'var(--gray2)',   label: 'UNREADABLE' },
};
  ...
  return (
    <div onClick={onClose} style={{
      ...
      <div onClick={(e) => e.stopPropagation()} className="card card-p"
        style={{ maxWidth: 460, width: '100%', background: 'var(--bg2, #14101f)' }}>
