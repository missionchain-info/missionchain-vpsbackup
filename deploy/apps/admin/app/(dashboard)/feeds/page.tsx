...
const STATUS_META: Record<string, { label: string; color: string }> = {
  draft: { label: 'DRAFT', color: '#848890' },
  pending_review: { label: 'PENDING', color: '#849ED4' },
  approved: { label: 'APPROVED', color: '#4A8FDA' },
  published: { label: 'PUBLISHED', color: '#3fbf7f' },
  rejected: { label: 'REJECTED', color: '#C05A67' },
};
...
const inp: React.CSSProperties = {
  width: '100%', padding: '9px 12px', borderRadius: 8, background: 'var(--bg2, #0E2148)',
  border: '1px solid var(--border, #28303F)', color: 'var(--white, #ECF0F7)', fontSize: 14, marginTop: 4,
};
const lbl: React.CSSProperties = { fontSize: 12, color: 'var(--muted, #8F97A8)', fontWeight: 600 };
  ...
  return (
    <div style={{ maxWidth: 1100, margin: '0 auto' }}>
      <div style={{ display: 'flex', alignItems: 'baseline', gap: 12, marginBottom: 6 }}>
        <h1 style={{ fontSize: 24, color: 'var(--gold, #C9A34C)', margin: 0 }}>📰 Feeds — Public Content</h1>
        <span style={{ fontSize: 12, color: 'var(--muted)' }}>Compose · AI-check · review · publish to app.missionchain.io/feeds</span>
      </div>

      <div style={{ background: 'rgba(201,163,76,.07)', border: '1px solid var(--border,#28303F)', borderRadius: 10, padding: '10px 14px', fontSize: 12.5, color: 'var(--muted)', marginBottom: 18 }}>
      ...
      {/* ── FORM ── */}
      <div style={{ background: 'var(--bg2, #0E2148)', border: '1px solid var(--border,#28303F)', borderRadius: 14, padding: 20, marginBottom: 26 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 120px 90px', gap: 12, marginBottom: 12 }}>
          ...
          {highRisk && <div style={{ alignSelf: 'end', fontSize: 11, color: '#E0B34A', fontWeight: 700 }}>⚠ SENSITIVE</div>}
        </div>
        ...
        {ai && ai.enabled && !ai.error && (
          <div style={{ marginTop: 12, background: 'rgba(74,143,218,.06)', border: '1px solid #2F465E', borderRadius: 10, padding: 14 }}>
            <div style={{ fontSize: 12, color: '#7FB5EF', fontWeight: 700, marginBottom: 8 }}>AI RESULT</div>
            {(ai.complianceFlags?.length ?? 0) > 0 && (
              <div style={{ marginBottom: 10, padding: '8px 10px', background: 'rgba(192,90,103,.12)', border: '1px solid #7A3A42', borderRadius: 8 }}>
                <b style={{ color: '#e88', fontSize: 12 }}>⚠ COMPLIANCE WARNINGS:</b>
                <ul style={{ margin: '6px 0 0', paddingLeft: 18, fontSize: 12.5, color: '#F0C0C6' }}>
            ...
            <div style={{ fontSize: 12, color: 'var(--gray,#C4CBD8)' }}>
        ...
        items.length === 0 ? <p style={{ color: 'var(--muted)' }}>No content yet.</p> :
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            {items.map(it => {
              ...
              return (
                <div key={it.id} style={{ display: 'flex', alignItems: 'center', gap: 12, background: 'var(--bg2,#0E2148)', border: '1px solid var(--border,#28303F)', borderRadius: 10, padding: '10px 14px' }}>
                  ...
                  <button onClick={() => remove(it)} style={{ ...btn('mini'), color: '#e88', borderColor: '#7A3A42' }}>Delete</button>
                </div>
              );
            })}
          </div>}

      {toast && <div style={{ position: 'fixed', bottom: 24, left: '50%', transform: 'translateX(-50%)', background: '#141A26', color: 'var(--white)', border: '1px solid var(--gold)', borderRadius: 8, padding: '10px 20px', zIndex: 1000, fontSize: 13 }}>{toast}</div>}
    </div>
  );
}

function btn(kind: 'gold' | 'ghost' | 'ai' | 'mini'): React.CSSProperties {
  const base: React.CSSProperties = { cursor: 'pointer', borderRadius: 8, fontWeight: 600, fontSize: 13, padding: '9px 16px', border: '1px solid var(--border,#28303F)', background: 'transparent', color: 'var(--white,#ECF0F7)' };
  if (kind === 'gold') return { ...base, background: 'var(--gold,#C9A34C)', color: '#141A26', border: 'none' };
  if (kind === 'ai') return { ...base, border: '1px solid #3F6EA0', color: '#8FBDEF' };
