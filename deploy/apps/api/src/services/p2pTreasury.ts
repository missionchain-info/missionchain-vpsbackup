 ...
 */
import { Contract, JsonRpcProvider, formatUnits, Provider } from 'ethers'
 ...
 */
let stateProvider: Provider | null = null
async function getStateProvider(): Promise<Provider> {
  if (stateProvider) {
 ...
 */
async function canServeLogs(p: Provider): Promise<boolean> {
  try {
...
export async function getLogProvider(): Promise<Provider> {
