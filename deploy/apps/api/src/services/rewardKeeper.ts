...
const EMISSION_ABI = [
  ...
  // EmissionControllerV2 widened this event with `activeLicences` and `elapsed`. The
  // signature is part of the topic hash, so a stale copy here matches NOTHING — silently,
  // with no error — and the two MIC pools below simply stop being notified. This line and
  // packages/sdk/abis/EmissionController.json must move together.
  'event DailyDistributed(uint256 day, uint256 activeLicences, uint256 elapsed, uint256 totalMinted, uint256 toMiners, uint256 toStaking, uint256 toDAO, uint256 toCommunityNFT, uint256 toMFPReward)',
] as const
