...
const ASSET_CONFIGS: AssetConfigExt[] = [
  {
    ...
    description: 'Mission Founders Pass \u00D710', color: '#407DFB',
  ...
  { id: 'LUMINARY', label: 'Luminary', icon: '\uD83D\uDC8E', tokenStandard: 'ERC-721', description: 'Community NFT \u00D75.0', color: '#93AAD8', reason: 'Coming soon' },
]
      ...
      <div className="p2p-order-top">
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          <span style={{
            ...
            background: isBuyer ? 'rgba(76,175,80,.15)' : 'rgba(212,164,55,.15)',
    ...
    <div className="p2p-modal-overlay" onClick={(e) => { if (e.target === e.currentTarget) onClose() }}>
      <div className="p2p-modal" onClick={e => e.stopPropagation()} style={{ maxWidth: 480 }}>
        ...
        <div className="p2p-modal-body">
          ...
          {soulLine && (
            <div style={{ padding: '12px 16px', background: 'rgba(20,42,87,.5)', borderRadius: 8, marginBottom: 8, fontStyle: 'italic', fontSize: '0.78rem', lineHeight: 1.55, color: 'var(--cream)' }}>
              {soulLine}
            </div>
          )}
          {verseLine && (
            <div style={{ padding: '12px 16px', background: 'rgba(20,42,87,.5)', borderRadius: 8, marginBottom: 16, fontSize: '0.74rem', color: 'var(--gray2)', lineHeight: 1.55 }}>
              {verseLine}
            </div>
          )}
