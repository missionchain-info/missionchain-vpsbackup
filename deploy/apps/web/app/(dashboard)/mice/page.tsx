    ...
    <>
      ...
      {buyResult && (
        <div style={{
          ...
          background: buyResult.ok ? 'rgba(76,175,80,.12)' : 'rgba(244,54,78,.12)',
          border: '1px solid ' + (buyResult.ok ? 'rgba(76,175,80,.3)' : 'rgba(244,54,78,.3)'),
          display: 'flex', alignItems: 'center', gap: 8, fontSize: '0.7rem',
        }}>
          <span style={{ flex: 1, color: buyResult.ok ? '#66BB6A' : '#EF5064' }}>{buyResult.msg}</span>
      ...
      */}
      {buyResult?.ok && (
        <div style={{
          ...
          background: 'rgba(240,190,74,.10)', border: '1px solid rgba(240,190,74,.32)',
        }}>
          <div style={{ fontSize: '0.78rem', fontWeight: 700, color: 'var(--gold, #F0BE4A)', marginBottom: 6 }}>
            Next step — activate to start earning
          </div>
          <div style={{ fontSize: '0.7rem', lineHeight: 1.75, color: 'var(--gray, #B9BFC6)' }}>
          ...
          <Link
            ...
            style={{
              ...
              background: 'var(--gold, #F0BE4A)', color: '#091530', fontWeight: 700,
      ...
      <div className="card" style={{ marginBottom: 16 }}>
        ...
        <p style={{ fontSize: '0.7rem', color: '#D4C298', lineHeight: 1.5, marginBottom: 12 }}>
        ...
        {!address ? (
          <div style={{
            padding: 14, borderRadius: 10, background: 'rgba(26,36,58,0.50)',
            border: '1px solid rgba(212,155,23,0.18)', fontSize: '0.7rem', color: '#D4C298',
          }}>
            Connect your wallet to see your referral network and volume.
          </div>
        ) : netLoading ? (
          <div style={{ fontSize: '0.7rem', color: '#D4C298', padding: 14 }}>Loading your network…</div>
        ) : !team ? (
          <div style={{
            padding: 14, borderRadius: 10, background: 'rgba(26,36,58,0.50)',
            border: '1px solid rgba(212,155,23,0.18)', fontSize: '0.7rem', color: '#D4C298',
            ...
            <Link href="/network" style={{ color: '#F5CC6E' }}>Building / My Community</Link> to retry.
          </div>
        ) : (
          <>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: 10 }}>
              <Stat label="F1 Direct Referrals" value={fmt(team.f1Members)} sub="members you introduced" accent="#F5CC6E" />
              <Stat label="F1 Team Volume" value={'$' + fmt(team.f1Volume)} sub="USDT purchased by F1" accent="#66BB6A" />
              <Stat label="F2 Indirect Referrals" value={fmt(team.f2Members)} sub="members introduced by your F1" accent="#A4BAE6" />
            ...
            */}
            ...
            <div style={{ marginTop: 8, fontSize: '0.6rem', color: '#B8AD94', lineHeight: 1.5 }}>
              Volume figures cover all qualifying sales (PreSale and MICE combined), not MICE alone.
            </div>
          </>
        )}
        <div style={{ marginTop: 10, fontSize: '0.6rem', color: '#B8AD94', fontStyle: 'italic' }}>
          See <Link href="/network" style={{ color: '#F5CC6E' }}>Building / My Community</Link> for full referral tree & Team Bonus.
        </div>
      </div>

      <div style={{ fontSize: '0.65rem', color: '#B8AD94', textAlign: 'center', padding: '12px', fontStyle: 'italic' }}>
        For mining stats, pool emission, and your active MICE rewards → see <Link href="/mining" style={{ color: '#F5CC6E' }}>Mining Pool</Link>.
      </div>
    </>
  )
}
...
function Stat({ label, value, sub, accent }: { label: string; value: string; sub?: string; accent?: string }) {
  return (
    <div style={{
      ...
      background: 'rgba(26,36,58,0.50)',
      border: '1px solid rgba(212,155,23,0.18)',
    }}>
      <div style={{ fontSize: '0.6rem', color: '#D4C298', marginBottom: 4, textTransform: 'uppercase', letterSpacing: '0.05em' }}>{label}</div>
      <div style={{ fontSize: '1.05rem', fontWeight: 700, color: accent || '#F5CC6E' }}>{value}</div>
      {sub && <div style={{ fontSize: '0.6rem', color: '#B8AD94', marginTop: 2 }}>{sub}</div>}
    </div>
  )
}
