    ...
    <div style={{minHeight:'100vh', background:'var(--bg)', color:'var(--white)', padding:'0 16px 24px', overflowY:'auto'}}>
      ...
      <div style={{maxWidth:760, margin:'0 auto', paddingTop:24}}>
        ...
        <nav style={{display:'flex', gap:8, flexWrap:'wrap', justifyContent:'center', marginBottom:20}}>
          {PILLARS.map(p => (
            <button key={p.key} onClick={()=>setPillar(p.key)} style={{padding:'6px 14px', borderRadius:999, cursor:'pointer', fontSize:13, border:'1px solid var(--border)', background: pillar===p.key ? 'var(--gold)' : 'transparent', color: pillar===p.key ? '#141A26' : 'var(--white)'}}>{p.label}</button>
          ))}
        </nav>
