...
export default function MicP2PPanel({ address }: { address?: string }) {
  ...
  return (
    <div className="nft-section-card">
      ...
      {msg ? (
        <div className="nft-pool-note" style={{ color: msg.ok ? '#7ddc9a' : '#FF8F9D' }}>
          {msg.text}
          {msg.hash && (
            <>
              {' — '}
      ...
      {total > 0 ? (
        <div className="nft-pool-note">
          ...
          {total < Number(cfg.minPriceUsdt) ? (
            <> <span style={{ color: '#FF8F9D' }}>Below the ${cfg.minPriceUsdt} minimum for a listing.</span></>
          ) : null}
          {/*
