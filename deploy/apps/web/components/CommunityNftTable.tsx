...
const TIER_COLOR: Record<number, string> = { 1: '#29B6F6', 2: '#AB47BC', 3: '#C084D4' }
...
export default function CommunityNftTable({ address, txHashes = {} }: {
  ...
  if (error) {
    return <div className="nft-pool-note" style={{ color: '#ffb400' }}>{error}</div>
  }
  ...
  return (
    <div className="nft-section-card" style={{ marginTop: 14 }}>
              ...
              return (
                <tr key={r.tokenId} style={{ borderTop: '1px solid var(--border)' }}>
                  ...
                  <td style={{
                    ...
                    color: left.expired ? '#ff8f8f' : undefined,
                  }}>
                    {left.text}
                  </td>
      ...
      {note && (
        <div className="nft-pool-note" style={{ color: note.ok ? '#7ddc9a' : '#ff8f8f', marginTop: 8 }}>
          {note.text}
        </div>
      )}
