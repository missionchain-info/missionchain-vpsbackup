...
export default function SwapPanel({ onDone }: { onDone?: () => void }) {
  ...
  const poolAddr = CONTRACTS.swapPool
  ...
  /**
   * When buying is scheduled to open. The 23,500,000 MIC that seeds the pool is held in
   * ListingReserveVault under a 7-day withdrawal cooldown that ends at this moment.
   *
   * This is the announcement, not the gate. The gate is `pool.seeded`, read from the
   * contract — so if seeding runs late the notice stays up, and the moment it lands the
   * panel opens on its own without anyone editing a date.
   */
  const SWAP_OPENS_AT = Date.UTC(2026, 7, 24, 6, 57, 45)   // 24 Aug 2026, 06:57:45 UTC
  const opensAtText = new Date(SWAP_OPENS_AT).toUTCString().replace('GMT', 'UTC')
  const msLeft = SWAP_OPENS_AT - Date.now()
  const waitText = msLeft <= 0
    ? 'Opening shortly.'
    : msLeft < 86_400_000
      ? `About ${Math.max(1, Math.round(msLeft / 3_600_000))} hour${Math.round(msLeft / 3_600_000) === 1 ? '' : 's'} to go.`
      : `About ${Math.round(msLeft / 86_400_000)} day${Math.round(msLeft / 86_400_000) === 1 ? '' : 's'} to go.`
  ...
  // The banner above already gives the date and the wait; repeating it inline said the
  // same sentence twice on one screen. Keep `blocked` set — it is what disables the
  // button — but let the banner do the explaining.
  else if (!pool.seeded) blocked = 'Nothing can be traded until the pool is funded with MIC.'
  ...
  const awaitingSeed = live && !!pool && !pool.seeded && !readFailed

  return (
    <div className="swap-panel">
      {/* Before the pool holds MIC there is nothing to trade, and saying so plainly beats
          a disabled button with no explanation. The date comes from the vault's own
          7-day cooldown; the panel still opens on `pool.seeded`, so this notice cannot
          outlive the thing it describes. */}
      {awaitingSeed && (
        <div className="swap-pending" role="status">
          <div className="swap-pending-t">Swap opens {opensAtText}</div>
          <div className="swap-pending-d">
            The pool is deployed and already collecting liquidity, but it holds no MIC
            yet {'\u2014'} so there is nothing to buy until it is funded. {waitText}
          </div>
        </div>
      )}
      ...
      <button className={'swap-go' + (awaitingSeed ? ' pending' : '')} disabled={!canSwap} onClick={doSwap}>
        {busy === 'approve' ? 'Approving…'
          : busy === 'swap' ? 'Swapping…'
          : awaitingSeed ? 'Opens ' + new Date(SWAP_OPENS_AT).toUTCString().slice(5, 16) + ' UTC'
